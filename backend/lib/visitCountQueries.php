...
function getVisitCount($con, $survey_id, $student_id) {
  $ret_val = 0;
  $stmt = $con->prepare("SELECT visit_count FROM student_visit_data WHERE student_id = ? AND survey_id = ?");
...
function createFirstVisit($con, $survey_id, $student_id) {
  ...
  $stmt = $con->prepare("INSERT INTO student_visit_data (student_id, survey_id, visit_count, last_visit) VALUES (?, ?, 1, ?)");
...
function updateVisitCount($con, $survey_id, $student_id, $visit_count) {
  $current_timestamp = date('Y-m-d H:i:s');
  $stmt = $con->prepare("UPDATE student_visit_data SET visit_count = ?, last_visit = ? WHERE student_id = ? AND survey_id = ?");
