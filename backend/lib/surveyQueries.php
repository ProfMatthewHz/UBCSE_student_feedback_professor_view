...
{
    $sql = 'SELECT courses.name AS course_name, surveys.name AS survey_name, surveys.id, surveys.start_date, surveys.end_date, COUNT(reviews.id) AS review_count, COUNT(evals.id) AS eval_count
            FROM surveys
            INNER JOIN courses ON courses.id = surveys.course_id 
            INNER JOIN reviews ON reviews.survey_id = surveys.id
    ...
    // Adjust the GROUP BY clause to include all columns from the SELECT list that are not aggregated
...
function createClosedQueryReviewer($con, $date_clause)
{
    ...
    // Adjust the GROUP BY clause to include all non-aggregated columns
    $sql .= ' GROUP BY courses.name, surveys.name, surveys.id, surveys.start_date, surveys.end_date';
...
{
    $sql = 'SELECT courses.name, surveys.name AS survey_name, surveys.id, surveys.start_date, surveys.end_date, COUNT(evals.id) AS evals_count
            FROM surveys
            INNER JOIN courses ON courses.id = surveys.course_id 
            INNER JOIN reviews ON reviews.survey_id = surveys.id
            LEFT JOIN evals ON evals.review_id = reviews.id
