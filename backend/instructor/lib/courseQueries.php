...
function getSurveysFromSingleCourse($con, $course_id){
  ...
  $stmt = $con->prepare('SELECT name, start_date, end_date, rubric_id, surveys.id, surveys.survey_type_id, surveys.pm_weight, COUNT(eval_id) AS total, COUNT(evals.completed) AS completed
                         FROM surveys
